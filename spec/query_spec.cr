...
describe LuckyRecord::Query do
  ...
  describe "#join methods for associations" do
    ...
    it "inner join on has many" do
      ...
      query.to_sql.should eq ["SELECT posts.id, posts.created_at, posts.updated_at, posts.title, posts.published_at, posts.employee_id FROM posts INNER JOIN comments ON posts.id = comments.post_id"]
