...
describe LuckyRecord::Query do
  ...
  describe "#where" do
    it "chains wheres" do
      ...
      query.statement.should eq "SELECT #{user_columns} FROM users WHERE first_name = $1 AND last_name = $2"
    ...
    it "handles int" do
      ...
      query.statement.should eq "SELECT #{user_columns} FROM users WHERE id = $1"
  ...
  describe "#limit" do
    it "adds a limit clause" do
      ...
      query.statement.should eq "SELECT #{user_columns} FROM users LIMIT 2"
    end
  end
  ...
  describe "#order_by" do
    it "adds an order clause" do
      ...
      query.statement.should eq "SELECT #{user_columns} FROM users ORDER BY name ASC"
    end
  end
...
private def user_columns
  "users.id, users.created_at, users.updated_at, users.name, users.age, users.nickname, users.joined_at"
end
